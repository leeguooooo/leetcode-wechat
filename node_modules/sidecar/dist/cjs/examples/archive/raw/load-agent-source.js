"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.loadAgentSource = void 0;
const { makeCompiler } = require('frida-compile');
/**
 * See: https://github.com/frida/frida-compile/blob/ca091615186f83c0f2326f07fbfd4eac86056fd7/index.js#L31-L40
 */
async function loadAgentSource() {
    const compile = makeCompiler(require.resolve('./agent.ts'), {}, {});
    const result = await compile();
    const agentSource = result.bundle.toString();
    return agentSource;
}
exports.loadAgentSource = loadAgentSource;
//# sourceMappingURL=load-agent-source.js.map
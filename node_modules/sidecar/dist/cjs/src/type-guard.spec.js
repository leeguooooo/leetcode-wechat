...
(0, tstest_1.test)('guardNativeType()', async (t) => {
    ...
    for (const [designType, nativeType, shouldMatch] of DESIGN_NATIVE_PAIR_LIST) {
        if (shouldMatch) {
            t.doesNotThrow(() => (0, type_guard_js_1.guardNativeType)(nativeType)(designType), [
                'should not throw when match:',
                `"${designType?.name}" <> "${nativeType}"`,
            ].join(' '));
        }
        else {
            t.throws(() => (0, type_guard_js_1.guardNativeType)(nativeType)(designType), [
...
(0, tstest_1.test)('guardPointerType()', async (t) => {
    ...
    for (const [designType, pointerType, shouldMatch] of DESIGN_POINTER_PAIR_LIST) {
        if (shouldMatch) {
            t.doesNotThrow(() => (0, type_guard_js_1.guardPointerType)([pointerType])(designType), [
                'should not throw when match:',
                `"${designType?.name}" <> "${pointerType}"`,
            ].join(' '));
        }
        else {
            t.throws(() => (0, type_guard_js_1.guardPointerType)([pointerType])(designType), [
                'should throw when does not match:',
                `"${designType?.name}" <> "${pointerType}"`,
            ].join(' '));
        }

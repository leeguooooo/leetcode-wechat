...
(0, tstest_1.test)('guard ret type', async (t) => {
    ...
    for (const [method, nativeType, pointerType, shouldMatch] of EXPECTED_RESULTS) {
        if (shouldMatch) {
            (0, guard_ret_type_js_1.guardRetType)(test, method, nativeType, [pointerType]);
            t.pass('should not throw for method/nativeType: ' + method + '/' + nativeType);
        }
        else {
            t.throws(() => (0, guard_ret_type_js_1.guardRetType)(test, 'method', nativeType, [pointerType]), 'should throw for method/nativeType: ' + method + '/' + nativeType);
        }
    }
});

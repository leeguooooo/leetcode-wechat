...
const sourceHandler = async ({ file, name, }) => {
    log.verbose('sidecar-dump <source>', 'file<%s>%s', file, name
        ? `, name<${name}>`
        : '');
    ...
    if (!name) {
        ...
        log.silly('sidecar-dump <source>', 'detected class name: "%s"', name);
    }
    const runFuncCode = [
        '(async () => {',
        [
    ...
    const agentSource = await executeWithContext(runFuncCode, {
        buildAgentSource,
        getMetadataSidecar,
        url: fileUrl.href,
    });

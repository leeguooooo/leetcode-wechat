...
import { Ret, RET_SYMBOL, } from '../../ret.js';
...
test('update & get call target metadata', async (t) => {
    ...
    const rpcDescriptor = updateRpcDescriptor(Test, 'testMethod', descriptor);
    ...
    t.notOk(Test[DEBUG_CALL_RET_ERROR], 'should not trigger error if the method returns "Ret()"');
});
test('method to be proxyed must retur "Ret()"', async (t) => {
    ...
    updateRpcDescriptor(Test, 'method', descriptor);
    await new Promise(setImmediate);
    t.ok(Test[DEBUG_CALL_RET_ERROR], 'should trigger error if the method does not return "Ret()" (We can safely ignore the Error message above this test)');
});

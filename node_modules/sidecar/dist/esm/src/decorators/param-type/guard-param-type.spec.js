...
test('guard parame type', async (t) => {
    ...
    for (const [nativeType, pointerType, shouldMatch] of EXPECTED_RESULTS) {
        if (shouldMatch) {
            guardParamType(test, 'method', 0, nativeType, [pointerType]);
            t.pass('should not throw for nativeType: ' + nativeType);
        }
        else {
            t.throws(() => guardParamType(test, 'method', 0, nativeType, [pointerType]), 'should throw for nativeType: ' + nativeType);
        }
    }
});
